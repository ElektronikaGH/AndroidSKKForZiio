...
public class SKKDictionary {
	...
	protected SKKDictionary(String dic) {
		...
		try {
			...
			if (mRecID == 0) {
				SKKUtils.dlog("Dictionary not found: " + mDicFile);
			}

			mBTree = BTree.load(mRecMan, mRecID);	    
		} catch (IOException e) {
			SKKUtils.dlog("Error: " + e.toString());
		}
	}
	...
	public ArrayList<String> getCandidates(String key) {
		...
		try {
			...
			if (va_array.length <= 0) {
				SKKUtils.dlog("Invalid value found: Key=" + key + " value=" + value);
				return null;
			}

			// va_array[0]ÇÕèÌÇ…ãÛï∂éöóÒÇ»ÇÃÇ≈1Ç©ÇÁénÇﬂÇÈ
	...
	public void findKeys(String key, boolean isKanji, ArrayList<String> list) {
		...
		try {
			...
			// ç≈èâÇÃàÍÇ¬Ç™keyÇ∆ìØÇ∂èÍçálistÇ…í«â¡ÇµÇ»Ç¢
